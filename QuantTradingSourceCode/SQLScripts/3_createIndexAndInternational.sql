...
(@Date,open,high,low,close,@trded,@turnover)
set timestamp=STR_TO_DATE(@Date,"%d-%b-%Y"), name ="NIFTY"; #Also try for BANKNIFTY, CNXIT, etc
# Insert the index data also into cmAdjPrice				#insert into cmAdjPrice (select name,timestamp,close from indices
insert ignore into cmAdjPrice							#Create a table to hold international stocks too
